 ...
 * (scripts/import-centers.mjs)와 함께 읽는다. 원본의 시군구명이 목록에 없는
 * 이름으로 오면 스크립트가 경고를 찍고 그 행을 건너뛴다. 값이 조용히
 * 사라지지 않게 하려는 것이므로, 경고가 나오면 JSON 을 고친 뒤 다시 적재할 것.
 *
 * yoyang 과 같은 파일을 쓴다. 두 사이트가 같은 행정구역 체계를 다루므로
 * 한쪽을 고치면 다른 쪽도 함께 확인할 것.
 */
